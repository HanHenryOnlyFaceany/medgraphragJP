...
async def extract_entities(
    ...
    if entity_vdb is not None:
        ...
        print("here we are------------------- data for vdb",data_for_vdb)
        if data_for_vdb:
...
def parse_response_to_json(response):
    # Split the response into sections based on the header pattern
    ...
    for section in sections:
        if section.startswith("### "):
            current_key = section[4:].strip().lower().replace(" ", "_")
        elif current_key:
            parsed_data[current_key] = section.strip()
