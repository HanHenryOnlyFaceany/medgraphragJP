...
"""
    ...
    def get_default_schema(self, data: DataPoint):
        # 获取默认schema的方法。
        # 1. 首先对输入数据进行预处理（如分块、设置print_schema等）。
        # 2. 从配置文件中获取默认schema，并设置到data对象中。
        # 3. 记录当前方法名和schema到抽取轨迹中，便于后续追踪。
        # 4. 返回更新后的data对象。
    ...
    def get_retrieved_schema(self, data: DataPoint):
        # 获取检索schema的方法。
        # 1. 对输入数据进行预处理。
        # 2. 根据data.output_schema字段，从schema仓库中获取对应的schema类。
        # 3. 如果能找到schema类，则序列化该schema，并与默认schema拼接后设置到data对象中，同时记录轨迹。
        # 4. 如果找不到schema类，则回退到默认schema。
        # 5. 返回更新后的data对象。
