...
class ExtractionAgent:
    ...
    def extract_information_direct(self, data: DataPoint):
        # 直接信息抽取方法。
        # 1. 首先根据任务类型和约束条件对 data 进行处理。
        # 2. 针对每个文本分块，调用大模型进行信息抽取（OneKE模型和其他模型处理方式略有不同）。
        # 3. 将每个分块的抽取结果收集到 result_list。
        # 4. 记录本方法名和抽取结果到抽取轨迹中。
        # 5. 返回更新后的 data 对象。
    ...
    def extract_information_with_case(self, data: DataPoint):
        # 基于案例的信息抽取方法。
        # 1. 首先根据任务类型和约束条件对 data 进行处理。
        # 2. 针对每个文本分块，先从案例库中检索相似的优秀案例作为示例。
        # 3. 调用大模型进行信息抽取，输入中包含检索到的案例示例。
        # 4. 将每个分块的抽取结果收集到 result_list。
        # 5. 记录本方法名和抽取结果到抽取轨迹中。
        # 6. 返回更新后的 data 对象。
        data = self.__get_constraint(data)
        result_list = []
    ...
    def summarize_answer(self, data: DataPoint):
        # 汇总多分块抽取结果的方法。
        # 1. 如果没有抽取结果，直接返回原始 data。
        # 2. 如果只有一个分块结果，直接作为最终预测结果。
        # 3. 如果有多个分块结果，则调用大模型对所有结果进行汇总总结，得到最终预测结果。
        # 4. 记录本方法名和最终结果到抽取轨迹中。
        # 5. 返回更新后的 data 对象。
