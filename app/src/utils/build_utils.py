...
def get_top_k_triples(n4j, gids, query_embedding, top_k=10):
    ...
    # 3. 收集相关节点的chunk_id去重复，然后根据chunk_id和gid获取chunk节点，提取出
    # chunk_ids = set()
    # for triple in triples_result:
    #     chunk_ids.add(triple['subject_chunk_id'])
    # # 4. 根据chunk_id和gid获取chunk节点，提取出chunk节点的内容
    # chunks = []
    # for chunk_id in chunk_ids:
    #     chunk_query = """
    #     MATCH (c:chunk)
    #     WHERE c.chunk_id = $chunk_id AND c.gid = $gid
    #     RETURN c.content AS content
    #     """
    #     chunk_result = n4j.query(chunk_query, {"chunk_id": chunk_id, "gid": triple['document_id']})
    #     if chunk_result:
    #         chunks.append(chunk_result[0]['content'])
    ...
    # # 3. 对每个相关gid，获取其内部节点关系（暂时不需要，太庞大了，也可以，后续看测试效果）
    # node_relations = {}
    ...
    #     # 按照ret_context函数的方式处理结果
    #     cont = []
    #     for r in res:
    #         cont.append(r['NodeId1'] + r['relType'] + r['NodeId2'])
        
    #     node_relations[gid] = cont
    ...
    #     # 按照link_context函数的方式处理结果
    #     cont = []
    #     for r in res:
    #         for ind, connection in enumerate(r["Connections"]):
    #             cont.append("Reference " + str(ind) + ": " + r["NodeId1"] + "has the reference that" + r['Mid'] + connection['RelationType'] + connection['Oid'])
        
    #     reference_relations[gid] = cont
